    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # Mask the password
                profile[0]['password'] = '******'  # Mask password

                # Fetch user uploads
                cursor.execute('SELECT * FROM uploads WHERE user_id = %s', (profile[0]['id'],))
                uploads = dict_fetch_all(cursor)

                # Attach uploads to profile
                profile[0]['uploads'] = uploads
                
                return jsonify(profile[0]), 200
...
def download_record(record_id):
    ...
    try:
        ...
        y_position -= 20
        c.drawString(100, y_position, f"File Size: {upload['file_size']} MB")
    except Exception as e:
        print("-------->" + str(e))
